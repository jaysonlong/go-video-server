$(document).ready(function () {
  ...
  initPage(function () {
    if (listedVideos !== null && listedVideos.length > 0) {
    ...
    $("#submit-comment").on('click', function () {
      if (!currentVideo) {
        popupErrorMsg("there is not a video");
        return
      }
  ...
  $("#uploadform").on('submit', function (e) {
    ...
    createVideo(vname, function (res, err) {
      ...
      popupNotificationMsg("Uploading video... ", 3000)
    });
  });
  ...
  $(".video-item").click(function () {
    var url = 'http://' + window.location.hostname + ':8080/videos/' + this.id
...
function initPage(callback) {
  getUserId(function (res, err) {
    if (err != null) {
      ...
      console.log('Encountered error when loading user id');
      setCookie("session", "", -1)
      setCookie("username", "", -1)
...
function popupNotificationMsg(msg, time = 2000) {
...
function popupErrorMsg(msg, time = 2000) {
